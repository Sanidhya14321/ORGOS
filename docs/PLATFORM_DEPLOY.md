...
**Optional Qdrant (vector storage instead of Postgres `embeddings`):** set `QDRANT_URL` (REST base, e.g. `http://localhost:6333`). Optional `QDRANT_API_KEY` (Qdrant Cloud). Optional `QDRANT_COLLECTION` (default `orgos_embeddings`). Ingest then upserts chunk vectors to Qdrant; hybrid/vector search queries Qdrant first (`apps/api/src/services/qdrantVectorStore.ts`, `ragSearchClient`). You still need an embedding API (today: `OPENAI_API_KEY` + `embedTexts`) — Qdrant only stores/search vectors. First ingest creates the collection with the embedding dimension from the model (typically 1536). Changing embedding size later requires a new collection or re-index. CEO document archive removes points for that document id in Qdrant when configured.
