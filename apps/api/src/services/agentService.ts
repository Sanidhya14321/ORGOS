...
import { buildRoutingCacheKey, getRoutingPromptCache, setRoutingPromptCache } from "./promptCache.js";
import { fetchRoutingMemoryContext } from "./routingMemory.js";
...
export async function suggestRoutingForTask(
  ...
  let userQuery = fastify.supabaseService
    .from("users")
    .select("id, role, skills, open_task_count, department")
    .eq("status", "active")
  ...
  const candidates = (candidatesResult.data ?? [])
    .map((user) => ({
      ...
      department: typeof (user as { department?: unknown }).department === "string"
  ...
  const deptNames = candidates
    .map((candidate) => candidate.department)
    .filter((value): value is string => typeof value === "string" && value.trim().length > 0);

  const cacheKey = task.org_id
  ...
  if (cacheKey) {
    const cached = await getRoutingPromptCache<AgentRoutingSuggestion>(fastify, cacheKey);
    if (cached && Array.isArray(cached.suggestions)) {
      const allowedAssignees = new Set(candidates.map((candidate) => candidate.id));
      const filteredCached = cached.suggestions.filter((suggestion) => allowedAssignees.has(suggestion.assigneeId));
  ...
  if (cacheKey) {
    await setRoutingPromptCache(fastify, cacheKey, { suggestions: filtered });
  }
