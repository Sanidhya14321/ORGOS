...
export default function Stepper({
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col items-center justify-center p-4 sm:aspect-[4/3] md:aspect-[2/1]" {...rest}>
      <div className={`mx-auto w-full max-w-md rounded-4xl shadow-xl ${stepCircleContainerClassName}`} style={{ border: '1px solid #222' }}>
        ...
        <StepContentWrapper isCompleted={isCompleted} currentStep={currentStep} direction={direction} className={`space-y-2 px-8 ${contentClassName}`}>
              ...
              {currentStep !== 1 && (
                <button
                  ...
                  className={`duration-350 rounded px-2 py-1 transition ${
                    currentStep === 1 ? 'pointer-events-none opacity-50 text-neutral-400' : 'text-neutral-400 hover:text-neutral-700'
                  }`}
                  {...backButtonProps}
                >
                  {backButtonText}
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators = false }: StepIndicatorProps) {
  ...
  return (
    <motion.div onClick={handleClick} className={`relative outline-none focus:outline-none ${disableStepIndicators ? 'pointer-events-none opacity-50' : 'cursor-pointer'}`} animate={status as any} initial={false}>
      <motion.div
        variants={{
      ...
      >
        {status === 'complete' ? <CheckIcon className="h-4 w-4 text-black" /> : status === 'active' ? <div className="h-3 w-3 rounded-full bg-[#120F17]" /> : <span className="text-sm">{step}</span>}
      </motion.div>
    </motion.div>
  )
}
