...
async function seed(): Promise<void> {
  ...
  if (orgUpsert.error || !orgUpsert.data) {
    throw new Error(`Unable to upsert demo org: ${orgUpsert.error?.message ?? "unknown error"}`);
  }
  ...
  const goalsPayload = Array.from({ length: 10 }).map((_, index) => ({
    ...
    description: `Seeded goal for scenario ${index + 1}`,
    raw_input: `Deliver seeded initiative ${index + 1}`,
    status: "active",
    priority: index % 4 === 0 ? "high" : "medium",
    kpi: `KPI-${index + 1}`,
    deadline: new Date(Date.now() + (index + 14) * 86_400_000).toISOString().slice(0, 10),
    simulation: false
  }));
  ...
  const tasksPayload = Array.from({ length: 120 }).map((_, index) => {
    ...
    return {
      ...
      description: `Seeded task description ${index + 1}`,
      ...
      sla_status: index % 17 === 0 ? "at_risk" : "on_track"
    };
  });
