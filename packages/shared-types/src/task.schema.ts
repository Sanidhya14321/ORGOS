...
export const TaskSchema = z.object({
  ...
  // Unlimited depth to support arbitrary hierarchies
  ...
  // Track if priority was inherited from parent task
  priority_inherited_from: z.string().uuid().nullable().optional(),
  
  // NEW: Position-based assignment (replaces hardcoded roles)
  assigned_position_id: z.string().uuid().nullable().optional(),
  suggested_position_ids: z.array(z.string().uuid()).optional(),
  
  // OLD: Kept for backward compatibility (deprecated)
  // Backward-compatibility: keep assigned_role as optional legacy field
